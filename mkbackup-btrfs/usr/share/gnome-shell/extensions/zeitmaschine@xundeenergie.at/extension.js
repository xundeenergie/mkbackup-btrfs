...
let ConfFile = '/tmp/mkbackup-btrfs.conf.tmp';
...
var BackupManager = new Lang.Class({
	...
	_init: function() {
        ...
        log(ConfFile);
        let GF = Gio.File.new_for_path(ConfFile);
        log(ConfFile);
        //this._monitorConf = this.GF.monitor_file(Gio.FileMonitorFlags.NONE,null,null,null)
        let _monitorConf = GF.monitor_file(Gio.FileMonitorFlags.NONE,null)
        log(ConfFile);
        _monitorConf.connect("changed", Lang.bind(this, function(monitor, file, o, event) {
    ...
    _run_command: function(COMMAND) {
        ...
        return ByteArray.toString(output[1]).replace(/\n$/, "") + "";
    },
    ...
    _refresh_panel : function() {
        ...
        this.aout = ByteArray.toString(GLib.spawn_command_line_sync(
            this._getCommand(this.services.join(' '), 'is-active', 'system'))[1]).split('\n');
        //log(this.aout);

        active = this.aout.indexOf('active') >= 0

        var vout = ByteArray.toString(GLib.spawn_command_line_sync(
                this._getCommand(volumes.join(' '), 'is-active', 'system'))[1]).split('\n');
	...
	_refresh: function() {
        ...
        var eout = ByteArray.toString(GLib.spawn_command_line_sync(
            this._getCommand(this.services.join(' '), 'is-enabled', 'system'))[1]).split('\n');
        //log(eout);
